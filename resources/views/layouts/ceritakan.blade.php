...
<div class="bg-[#F8FAFC] min-h-screen pb-20">
    ...
    <div class="max-w-4xl mx-auto px-6 pt-12 pb-8">
        ...
        <p class="text-gray-500">Suaramu sangat berarti. Setiap laporan akan kami jaga kerahasiaannya demi kenyamanan bersama.</p>
    </div>

    <div class="max-w-4xl mx-auto px-6">
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-8 md:p-12">
                <form action="/ceritakan/simpan" method="POST">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        ...
                        <div>
                            ...
                            <select name="kategori" class="w-full bg-gray-50 border border-gray-100 rounded-2xl px-5 py-4 focus:outline-none focus:border-[#2D5A7B] focus:bg-white transition-all appearance-none">
                    ...
                    <div class="bg-[#F0F7F7] p-6 rounded-[2rem] mb-10 flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-white rounded-full flex items-center justify-center text-[#2D5A7B] shadow-sm">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                            </div>
                            <div>
                                <h4 class="font-bold text-[#2D5A7B] text-sm">Kirim sebagai Anonim</h4>
                                <p class="text-[11px] text-gray-500">Namamu tidak akan diperlihatkan kepada siapapun.</p>
                            </div>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_anonymous" value="1" class="sr-only peer" checked>
                            <div class="w-14 h-7 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#2D5A7B]"></div>
                        </label>
                    </div>

                    <!-- Tombol Aksi -->
                    <div class="flex flex-col md:flex-row gap-4">
                        <button type="submit" class="flex-1 bg-[#2D5A7B] text-white py-4 rounded-2xl font-bold hover:bg-[#244a66] shadow-lg shadow-teal-900/10 transition-all flex items-center justify-center gap-3">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"/></svg>
                            Kirim Cerita Sekarang
                        </button>
                        <button type="button" class="px-10 py-4 bg-white border border-gray-200 text-gray-500 rounded-2xl font-bold hover:bg-gray-50 transition-all">
                            Simpan Draft
                        </button>
                    </div>
                </form>
