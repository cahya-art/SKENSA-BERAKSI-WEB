        ...
        <div class="w-full md:w-1/2 flex flex-col justify-center px-8 md:px-24 py-10">
            ...
            <form action="{{ route('register') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" required placeholder="Enter your full name" 
                            class="w-full pl-10 pr-4 py-3 bg-[#F0F7F7] border @error('name') border-red-500 @else border-transparent @enderror focus:border-teal-500 focus:bg-white rounded-xl outline-none transition-all">
                    </div>
                    @error('name')
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" /></svg>
                        </span>
                        <input type="email" name="email" value="{{ old('email') }}" required placeholder="Enter your email address" 
                            class="w-full pl-10 pr-4 py-3 bg-[#F0F7F7] border @error('email') border-red-500 @else border-transparent @enderror focus:border-teal-500 focus:bg-white rounded-xl outline-none transition-all">
                    </div>
                    @error('email')
                        <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="nis" value="{{ old('nis') }}" required placeholder="Enter your NIS number (e.g. 32355)" 
                            class="w-full pl-10 pr-4 py-3 bg-[#F0F7F7] border @error('nis') border-red-500 @else border-transparent @enderror focus:border-teal-500 focus:bg-white rounded-xl outline-none transition-all">
                    </div>
                    @error('nis')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="password" name="password" required placeholder="Enter your Password" 
                            class="w-full pl-10 pr-4 py-3 bg-[#F0F7F7] border @error('password') border-red-500 @else border-transparent @enderror focus:border-teal-500 focus:bg-white rounded-xl outline-none transition-all">
                    </div>
                    @error('password')
                        <span class="text-red-500 text-xs mt-1 block">{{ $message }}</span>
                    @enderror
